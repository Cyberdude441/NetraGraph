    ...
    <div className="flex h-full min-h-0 flex-col bg-[#064E3B] text-[#E2E8F0] transition-colors duration-200 select-none">
      ...
      <div className="space-y-4">
        ...
        <nav className="flex flex-col gap-1 pt-2">
          ...
          {NAV.map((item, index) => {
            const active =
              pathname === item.to && NAV.findIndex((navItem) => navItem.to === item.to) === index;
            return (
              <Link
                key={item.label}
                to={item.to}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative shrink-0" ref={profileRef}>
                <button
                  ...
                  className="flex items-center gap-2.5 whitespace-nowrap rounded-md border border-[#E5E7EB] bg-[#F8FAF8] px-3 py-1 text-left hover:border-[#16A34A] transition-colors"
