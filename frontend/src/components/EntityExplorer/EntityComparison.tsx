      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {entities.map((ent) => {
          ...
          return (
            <div
              key={ent.id}
              className="rounded-lg border border-[#E2E8F0] bg-white p-3 space-y-3 relative"
            >
              <button
                onClick={() => onRemoveEntity(ent.id)}
                className="absolute top-2.5 right-2.5 text-slate-500 hover:text-slate-800 p-0.5 rounded cursor-pointer"
              ...
              {/* Title & Role */}
              <div className="pr-6">
                <h3 className="font-sans text-xs font-bold text-slate-900 uppercase tracking-wide truncate">
                  {ent.name}
                </h3>
              ...
              {/* Threat Pill */}
              <div className="flex items-center justify-between border-t border-b border-[#E2E8F0]/80 py-1.5 font-mono text-[11px]">
                <span className="text-slate-500">Risk Score:</span>
                <span
                  className={cn(
                    "font-bold flex items-center gap-1",
                    isHigh ? "text-red-400" : risk >= 70 ? "text-amber-400" : "text-slate-700"
                  )}
                >
              ...
              <div className="space-y-1.5 font-mono text-[11px]">
                <div className="flex items-center justify-between">
                  ...
                  <strong className="text-slate-800">{ent.label}</strong>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-emerald-400">{ent.pageRankScore ? `${ent.pageRankScore}%` : "—"}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-slate-800 font-bold">{ent.degreeCount || 1}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Jurisdiction:</span>
                  <span className="text-slate-700 truncate max-w-[150px]">{ent.metadata.jurisdiction || "—"}</span>
                </div>
              </div>

              {/* Aliases */}
              {ent.metadata.alias && ent.metadata.alias.length > 0 && (
                <div className="border-t border-[#E2E8F0]/80 pt-1.5 text-[10px] font-mono">
                  <span className="text-slate-500 block mb-1">Aliases:</span>
                  <div className="flex flex-wrap gap-1">
                    {ent.metadata.alias.map((a, i) => (
                      <span key={i} className="rounded bg-[#F8FAFC] px-1.5 py-0.5 text-slate-700 border border-[#E2E8F0]">
                        "{a}"
                      </span>
                    ))}
                  </div>
