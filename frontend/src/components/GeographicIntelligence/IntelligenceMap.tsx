...
export function IntelligenceMap({
  ...
  return (
    <div className="relative w-full h-full bg-[#080B0F] overflow-hidden select-none font-sans rounded-lg border border-[#E2E8F0]">
      {/* Map Tactical HUD Overlay Bar */}
      <div className="absolute top-3 left-3 z-10 flex items-center gap-2 bg-white/90 backdrop-blur-md p-1.5 rounded-lg border border-[#E2E8F0] font-mono text-[10px]">
        <span className="flex size-2 rounded-full bg-emerald-400 animate-pulse" />
        <span className="font-bold text-slate-800 uppercase">
          Tactical Geospatial Grid · Synthetic Telemetry
        </span>
        <span className="text-slate-500">|</span>
        <span className="text-emerald-400">{locations.length} Monitored Facilities</span>
      </div>

      {/* Map Zoom / Pan Controls */}
      <div className="absolute top-3 right-3 z-10 flex flex-col gap-1 bg-white/90 backdrop-blur-md p-1 rounded-lg border border-[#E2E8F0]">
        <button
          onClick={handleZoomIn}
          className="p-1.5 text-slate-400 hover:text-emerald-300 hover:bg-slate-100 rounded transition-colors cursor-pointer"
          title="Zoom In"
        >
      ...
      >
        ...
        <g opacity="0.3" stroke="#E2E8F0" strokeWidth="1" fill="none" strokeDasharray="3 3">
          ...
          return (
            ...
            >
              ...
              <circle
                ...
                cy={pos.y}
                r={isSelected ? "11" : "8"}
                fill={isCrit ? "#EF4444" : isHigh ? "#F59E0B" : "#10B981"}
                stroke="#FFFFFF"
                strokeWidth="2.5"
                className="shadow-lg"
              />
