...
export function ContextManager({
  ...
  return (
    <div className="space-y-4 text-xs font-sans select-none">
      ...
      <div className="rounded border border-[#E2E8F0] bg-white p-3 space-y-1.5">
        <label className="text-[10px] font-mono uppercase font-bold text-slate-400 block flex items-center gap-1">
          <FolderSearch className="size-3 text-emerald-400" /> Active Investigation Docket
        </label>
        <select
          value={activeCaseId}
          onChange={(e) => onSelectCase(e.target.value)}
          className="w-full rounded border border-[#E2E8F0] bg-[#F8FAFC] px-2 py-1 text-xs text-emerald-300 font-mono outline-none cursor-pointer"
        >
      ...
      <div className="rounded border border-[#E2E8F0] bg-white p-3 space-y-2">
        ...
        <div className="space-y-1 font-mono text-[11px]">
          {pinnedEntities.map((ent) => (
            <div
              key={ent.id}
              className="rounded bg-[#F8FAFC] px-2 py-1 border border-[#E2E8F0] flex items-center justify-between"
            >
              <span className="text-slate-800 truncate">{ent.name}</span>
              <button
      ...
      <div className="rounded border border-[#E2E8F0] bg-white p-3 space-y-1.5 font-mono text-[10px]">
