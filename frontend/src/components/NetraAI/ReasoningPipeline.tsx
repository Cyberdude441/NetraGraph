    ...
    <div className="rounded-lg border border-[#E2E8F0] bg-white p-4 text-xs select-none space-y-3 font-sans shadow-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 font-mono text-[10px]">
        {steps.map((step) => (
          <div
            key={step.stepNumber}
            className="rounded border border-[#E2E8F0]/80 bg-white p-2.5 space-y-1 relative"
          >
            ...
            <div className="font-bold text-slate-800 truncate">{step.name}</div>
            <p className="text-[9px] text-slate-400 leading-tight font-sans line-clamp-2">
              {step.description}
            </p>

            {step.nodesScanned && (
              <div className="text-[8px] text-emerald-400 pt-0.5">
