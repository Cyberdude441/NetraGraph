  ...
  return (
    ...
    >
      <div className="space-y-4 font-sans select-none">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#E2E8F0] bg-white p-3.5 shadow-xl">
          <div className="flex items-center gap-2 font-mono text-xs">
            <FolderSearch className="size-4 text-emerald-400" />
            <span className="text-slate-400 font-bold uppercase">Select Investigation Docket:</span>
            <select
              value={selectedCaseId}
              onChange={(e) => setSelectedCaseId(e.target.value)}
              className="rounded border border-[#E2E8F0] bg-[#F8FAFC] px-2.5 py-1 text-xs text-emerald-300 font-mono outline-none cursor-pointer"
            >
