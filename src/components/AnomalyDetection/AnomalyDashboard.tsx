    ...
    <div className="space-y-4 font-sans select-none">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 font-mono text-xs">
        ...
        <div className="rounded border border-slate-800 bg-[#121820] p-3">
          ...
          <div className="text-xl font-bold text-slate-100">{alerts.length} Patterns</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2.5 font-mono text-xs">
        {[
          ...
          { id: "DEVICE_HOPPING", name: "Burner Devices", icon: Cpu, color: "text-cyan-400", count: alerts.filter(a => a.category === "DEVICE_HOPPING").length },
          { id: "COMMUNICATION_BURST", name: "Call Spikes", icon: PhoneCall, color: "text-sky-400", count: alerts.filter(a => a.category === "COMMUNICATION_BURST").length },
          ...
          return (
            <button
              ...
              className="rounded-lg border border-slate-800 bg-[#121820] p-3 flex items-center justify-between hover:border-slate-700 transition-colors cursor-pointer text-left"
            >
              <div className="flex items-center gap-2">
                <Icon className={cn("size-4", item.color)} />
                <span className="font-bold text-slate-200 text-[11px]">{item.name}</span>
              </div>
