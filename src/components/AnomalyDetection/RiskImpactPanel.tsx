...
export function RiskImpactPanel({ alert }: RiskImpactPanelProps) {
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-[#0E1318] p-4 text-xs select-none space-y-3 font-sans shadow-2xl">
      ...
      <div className="rounded border border-slate-800 bg-[#121820] p-3 flex items-center justify-between font-mono">
        <div>
          <span className="text-[9px] uppercase text-slate-500 block">Baseline Risk</span>
          <span className="text-base font-bold text-slate-300">{assessment.baselineRiskScore}/100</span>
        </div>
      ...
      <div className="space-y-1.5 font-mono text-[10px]">
        ...
        {assessment.contributions.map((c, i) => (
          <div
            key={i}
            className="rounded border border-slate-800 bg-[#161D24] p-2 flex items-center justify-between"
          >
            <div>
              <strong className="text-slate-200 block text-[11px]">{c.factorName}</strong>
      ...
      <div className="rounded border border-slate-800/80 bg-[#10161E] p-2 text-[9px] font-mono text-slate-500 flex items-start gap-1.5 leading-tight">
