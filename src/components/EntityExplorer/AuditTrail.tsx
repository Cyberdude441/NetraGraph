    ...
    switch (action) {
      ...
      case "ALIAS_ADDED":
        return {
          icon: Tag,
          text: "Alias Added",
          style: "bg-sky-950/60 border-sky-800 text-sky-300",
        };
    ...
    <div className="rounded-lg border border-slate-800 bg-[#0E1318] p-5 text-xs select-none space-y-4 font-sans shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded border border-slate-800 bg-[#161D24] px-3 py-1.5 text-xs font-mono font-semibold text-slate-200 hover:border-emerald-500 transition-colors cursor-pointer"
        >
            ...
            {logs.map((log) => {
              ...
              return (
                <tr key={log.id} className="hover:bg-[#121820] transition-colors">
                  ...
                  <td className="px-3 py-2.5 text-slate-300 max-w-xs truncate" title={log.details}>
                    {log.details}
                  </td>
                </tr>
              );
