    ...
    <div className="rounded-lg border border-slate-800 bg-[#0E1318] p-4 text-xs select-none space-y-4 font-sans shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="rounded border border-slate-800 bg-[#121820] p-3 space-y-2">
          ...
          <div className="space-y-1 font-mono text-[11px]">
            <div>
              ...
              <strong className="text-slate-100">{entityA.name}</strong>
            </div>
            <div>
              <span className="text-slate-500">Role: </span>
              <span className="text-slate-300">{entityA.role || entityA.label}</span>
            </div>
            <div>
              <span className="text-slate-500">Jurisdiction: </span>
              <span className="text-slate-300">{entityA.metadata.jurisdiction || "—"}</span>
            </div>
            <div>
        ...
        <div className="rounded border border-slate-800 bg-[#121820] p-3 space-y-2">
          ...
          <div className="space-y-1 font-mono text-[11px]">
            <div>
              <span className="text-slate-500">Name: </span>
              <strong className="text-slate-100">{entityB.name}</strong>
            </div>
            <div>
              <span className="text-slate-500">Role: </span>
              <span className="text-slate-300">{entityB.role || entityB.label}</span>
            </div>
            <div>
              <span className="text-slate-500">Jurisdiction: </span>
              <span className="text-slate-300">{entityB.metadata.jurisdiction || "—"}</span>
            </div>
            <div>
              <span className="text-slate-500">Aliases: </span>
              <span className="text-amber-300">
      ...
      <div className="rounded border border-slate-800 bg-[#141A21] p-3 space-y-2.5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono text-[11px]">
          <div className="rounded bg-[#10171F] p-2 border border-slate-800">
            ...
            <span className="text-sm font-bold text-sky-400">{matrix.nameSimilarity}%</span>
          </div>
          <div className="rounded bg-[#10171F] p-2 border border-slate-800">
      ...
      <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
        {onClose && (
          <button
            onClick={onClose}
            className="rounded border border-slate-800 bg-[#161D24] px-3 py-1.5 text-xs font-mono text-slate-400 hover:text-slate-200 cursor-pointer"
          >
            Cancel
          </button>
        )}
