...
export function LayoutSwitcher({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-[#0E1318] px-4 py-2 text-xs select-none">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 rounded bg-[#161D24] p-1 border border-slate-800">
          {layouts.map((l) => {
            const Icon = l.icon;
      ...
      <div className="flex items-center gap-2">
        ...
        {hasFocalNode && hopDistance > 0 && (
          <span className="rounded bg-sky-950/60 border border-sky-800/80 px-2 py-0.5 text-[10px] font-mono text-sky-300 flex items-center gap-1">
            <Sparkles className="size-3 text-sky-400" />
            Filtered on: <strong className="text-slate-100 truncate max-w-[120px]">{focalNodeName}</strong>
          </span>
        )}
      </div>
    </div>
