...
export function ShortestPathTracer({
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-[#0E1318] p-5 text-xs select-none space-y-4 font-sans shadow-2xl">
      ...
      {/* Target Selectors */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-[#121820] p-3 rounded-lg border border-slate-800">
        <div>
          <label className="text-[10px] font-mono uppercase font-bold text-sky-400 block mb-1">
          ...
          <select
            ...
            className="w-full rounded border border-slate-800 bg-[#161D24] px-2.5 py-1.5 text-xs text-slate-100 font-mono outline-none focus:border-sky-500 cursor-pointer"
          >
            {entities.map((e) => (
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded border border-slate-800 bg-[#161D24] px-2.5 py-1.5 text-xs text-slate-100 font-mono outline-none focus:border-amber-500 cursor-pointer"
          >
            {entities.map((e) => (
              <option key={e.id} value={e.id}>
                {e.name} ({e.label} · {e.id})
      ...
      {currentPath ? (
        <div className="space-y-4">
          ...
          {/* Hop Nodes Chain */}
          <div className="space-y-3 relative pl-6 before:absolute before:left-2.5 before:top-4 before:bottom-4 before:w-0.5 before:bg-slate-800">
                ...
                <div key={node.id} className="relative group">
                  ...
                  <div
                    className={cn(
                      ...
                      isFirst
                        ? "border-sky-500 bg-sky-950 text-sky-300 ring-2 ring-sky-500/40"
                        : isLast
                  ...
                  <div
                    ...
                    className="rounded-lg border border-slate-800 bg-[#121820] p-3 hover:border-sky-500 transition-all cursor-pointer flex items-center justify-between gap-3"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <strong className="font-bold text-slate-100 text-xs">
                          {node.name}
                        </strong>
                        <span className="rounded bg-[#161D24] px-1.5 py-0.2 font-mono text-[9px] text-slate-400 border border-slate-800">
                          {node.label}
                        </span>
                      </div>
                  ...
                  {nextEdge && (
                    <div className="my-1.5 ml-4 flex items-center gap-2 text-[10px] font-mono text-slate-400">
                      ...
                      <span className="rounded bg-[#161D24] px-2 py-0.5 text-amber-300 border border-slate-800 font-bold">
                        {nextEdge.label}
                      </span>
