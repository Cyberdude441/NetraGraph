"""Forensic Evaluation & Metric Computation for CSE-CIC-IDS2018."""
...
def compute_cybersecurity_metrics(
    ...
    """Computes ML benchmarks and cybersecurity operational threat metrics (FPR, FNR)."""
    ...
    metrics: Dict[str, Any] = {
        "accuracy": acc,
        "precision": float(precision_score(y_true, y_pred, average=avg_mode, zero_division=0)),
        "recall": float(recall_score(y_true, y_pred, average=avg_mode, zero_division=0)),
    ...
    if y_proba is not None:
        try:
            ...
            metrics["auc_computation_note"] = str(exc)

    return metrics


def print_evaluation_summary(metrics: Dict[str, Any], title: str = "CIC-IDS2018 MODEL EVALUATION") -> None:
    """Prints formatted SOC evaluation summary."""
    print("\n" + "=" * 65)
    print(f"  {title}")
    print("=" * 65)
    print(f"  Accuracy                : {metrics['accuracy'] * 100:.2f}%")
    print(f"  F1 Score                : {metrics['f1'] * 100:.2f}% (Weighted: {metrics['f1_weighted'] * 100:.2f}%)")
    ...
    print("-----------------------------------------------------------------")
    print("  SOC THREAT OPERATIONAL METRICS:")
    print(f"  False Positive Rate (FPR): {metrics['false_positive_rate'] * 100:.3f}% (Alert Noise Overhead)")
    print(f"  False Negative Rate (FNR): {metrics['false_negative_rate'] * 100:.3f}% (Undetected Breach Risk)")
    print("-----------------------------------------------------------------")
...
def evaluate_saved_artifact(artifact_dir: str | Path, test_df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    """Loads a saved NetraGraph artifact bundle and evaluates metrics."""
    import joblib
    loc = Path(artifact_dir)
    print(f"[Evaluator] Loading artifact from: {loc}")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CIC-IDS2018 Model Artifact")
    parser.add_argument("--artifact", required=True, help="Path to versioned artifact directory")
    args = parser.parse_args()

    evaluate_saved_artifact(args.artifact)
